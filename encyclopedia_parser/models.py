...
class EditionConfig(BaseModel):
    ...
    # === NEW: Edition-specific content rules ===

    # Allowed article types for this edition
    # e.g., 1st edition (1771) only has dictionary/treatise, no biography
    allowed_article_types: list[ArticleType] = Field(
        default_factory=lambda: list(ArticleType)
    )

    # Whether this edition includes biographical entries
    has_biography: bool = Field(default=True)

    # Whether this edition includes geographical entries
    has_geography: bool = Field(default=True)

    # Index volume number (if edition has an index volume)
    index_volume: Optional[int] = Field(default=None)

    # Edition-specific OCR artifact patterns (headwords to reject)
    ocr_artifacts: list[str] = Field(default_factory=list)

    # Edition-specific notes for documentation
...
EDITION_CONFIGS = {
    ...
    1771: EditionConfig(
        ...
        # 1st edition is Arts & Sciences only - NO biography or history of persons
        has_biography=False,
        has_geography=True,  # Has geographical entries
        ...
        notes="First edition focused on Arts & Sciences. Biography and History of persons were explicitly excluded."
    ),
    1778: EditionConfig(
        year=1778,
        name="Britannica 2nd",
        ...
        # 2nd edition added Biography and History
        has_biography=True,
        has_geography=True,
        notes="Added Biography and History. Volume 10 contains an Appendix with corrections."
    ),
    1797: EditionConfig(
        year=1797,
        name="Britannica 3rd",
    ...
    1842: EditionConfig(
        year=1842,
        name="Britannica 7th",
        volumes=22,  # 21 content + 1 index
        major_treatises=[],  # TODO: Populate from index
        index_volume=22,  # Volume 22 is the General Index
        notes="Has General Index in Volume 22 - the 'Rosetta Stone' for taxonomy."
    ),
    1860: EditionConfig(
        year=1860,
        name="Britannica 8th",
