#!/bin/bash
# One-time setup for EarlyModernNER on Vulcan (DRAC).
# Creates a dedicated venv, installs dependencies, downloads model + adapters.
#
# Usage: bash graphrag/plato/setup_ner_vulcan.sh

echo "=== EarlyModernNER Setup for Vulcan ==="

# Source CVMFS before strict mode (it uses unbound vars)
source /etc/profile.d/zz-cvmfs.sh

set -euo pipefail
module load python/3.11 cuda/12.6
echo "Modules loaded: python/3.11, cuda/12.6"

VENV_DIR="$HOME/ner_venv"
...
echo ""
echo "Test:"
echo "  source /etc/profile.d/zz-cvmfs.sh"
